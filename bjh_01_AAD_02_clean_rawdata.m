function [ALLEEG_clean] = bjh_01_AAD_02_clean_rawdata(ALLEEG_select,params)
%% cleans data using clean_rawdata and interpolates removed channels
% 
% input: ALLEEG_imp:    [struct array] structure array containing imported
...
% 
% output: ALLEEG_clean: [struct array] structure array containing cleaned 
%                       data of all participants
% 
% author: Björn Holtze
% date: 14.04.21

for s = 1:size(ALLEEG_select,2)
    
    % high-pass filter (only for comparison before/after with vis_artifacts)
    EEG_filt = clean_drifts(ALLEEG_select(s),params.highpass);
    
    % clean data using default clean_rawdata (default parameters)
    EEG_clean = clean_rawdata(ALLEEG_select(s),params.flatline,params.highpass,params.chancrit,...
        params.linenoise,params.burstcrit,params.wincrit);
