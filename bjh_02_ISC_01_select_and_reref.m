function [ALLEEG_ISC_select_and_reref] = bjh_02_ISC_01_select_and_reref(MAINPATH,CONFIGPATH)
%% selects first block, and references to linked mastoids
% epochs according to 10 minute blocks ([-10, 610]), removes DC
% offset (first 500 ms of each epoch), keeps only first 10 minute blocks 
% Lastly, the data is rereferenced to linked mastoid and channel L04A is 
% removed to have same number of channels on the left and right side.
% 
% input:    MAINPATH:   [string] path where .xdf files are stored
%           CONFIGPATH: [string] path where subj_info.mat is stored
%
% output:   ALLEEG_ISC_select_and_reref: 
%                       structure array containing EEG structures as 
%                       subfields (only first block and rereferenced)
%
% author: Björn Holtze
% date: 14.01.22
...
for s = 1:size(subj_info,1)
    ...
    % select first block of each participant
    EEG = pop_selectevent(EEG,'type',{'StartTrigger'},'epoch',1,...
    ...
    ALLEEG_ISC_select_and_reref(s) = EEG;
    clear EEG;
